...
def get_xy(dta_name, load_from_disk=False, binned=False):
    ...
    y = dta[target].values
    N = len(y)
    y = class2index(y, tuple(np.unique(y, return_counts=True)[0])) # transform class labels to a range between 0 and n-1    
